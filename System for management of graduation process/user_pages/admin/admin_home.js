...
function buildStudentsDiplomaTable(users, colors_config) {
    ...
    users.forEach(user => {
        ...
        cell29.innerHTML = user.student_alumni == 0 ? 'Не' : 'Да';
        i++;
    })
}
...
function dataRequestedGrown(data){
    const a=[["Заявка за тога","Брой студенти с такава заявка"],["Заявили",0],["Незаявили",0]];
    let rows=data.users;
    rows.forEach(row_data =>{
        console.log(row_data.grown_requested);
        switch(row_data.grown_requested){
...
function dataAlumni(data){
    const a=[["Участие в алумни клуб","Брой студенти с такава заявка"],["Ще участвам",0],["Няма да участвал",0]];
    let rows=data.users;
    rows.forEach(row_data =>{
        switch(row_data.student_alumni){
            case 1: 
                a[1][1]++;
                break;
            case 0:
...
function dataRequestedHat(data){
    const a=[["Заявка за шапка","Брой студенти с такава заявка"],["Заявили",0],["Незаявили",0]];
    let rows=data.users;
    rows.forEach(row_data =>{
        switch(row_data.hat_requested){
            case 1: 
                a[1][1]++;
                break;
            case null:
...
function ShowStatistic(){
    let whichTableId=document.getElementById("selectAnalyticsDiv").value;
    const whichDataToBeDisplayed=document.getElementById("selectDataToBeDisplayed").value;
    
    whichTableId="analytics"+whichTableId;
    ...
    fetch(`../../services/get_students_diploma.php`, {
        ...
        .then((data) => {
            if (data.error) {
                ...
                let dataForPie;
                switch(whichDataToBeDisplayed){
                    case '1':{
                        dataForPie=dataGradesToArray(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети с дадена оценки"));
                        break;
                    }
                    case '2':{
                        dataForPie=dataHasRightToArray(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети имащи право на диплома"));
                        break;
                    }
                    case '3':{
                        dataForPie=dataDegreeToArray(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети с дадена степен на образование"));
                        break;
                    }
                    case '4':{
                        dataForPie=dataMajorToArray(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети с дадена специалност"));
                        break;
                    }
                    case '5':{
                        dataForPie=dataRequestedGrown(data);
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети заявили тога")); 
                        break;
                    }
                    case '6':{
                        dataForPie=dataRequestedHat(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети заявили шапка"));
                        break;
                    }
                    case '7':{
                        dataForPie=dataAlumni(data); 
                        google.charts.setOnLoadCallback(drawChart(dataForPie,whichTableId,"Брой стундети заявили участие в алумни клубът"));
                        break;
                    }
                    default:{
                        break;
